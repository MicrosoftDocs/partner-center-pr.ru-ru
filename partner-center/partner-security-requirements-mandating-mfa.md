...
-   Все включенные пользователи, включая гостевые пользователи

Эти партнеры должны пройти проверку MFA для следующих областей:

- [Панель мониторинга центра партнеров](#partner-center-dashboard) (нацеливание на Q2 CY2020)
- [API центра партнеров](#partner-center-api) (нацеливание на Q2 CY2020)
...
2. На своей рабочей станции Trent) запускает новый сеанс браузера и переходит на страницу обзора панели мониторинга центра партнеров (которая не защищена с помощью MFA). Центр партнеров перенаправляет Trent) в Azure AD для входа.
...
4. Trent) пытается получить доступ к одной из страниц, защищенных с помощью MFA, в центре партнеров. Так как Trent) уже выполнил проверку MFA во время входа ранее, Trent) может получить доступ к защищенной странице MFA без необходимости повторного выполнения проверки MFA.
...
Общий интерфейс очень похож на сценарий, в котором клиент конечного клиента реализовал MFA для своих администраторов. Например, клиент клиента включил [Параметры безопасности Azure AD](https://docs.microsoft.com/azure/active-directory/fundamentals/concept-fundamentals-security-defaults), что требует наличия всех учетных записей с правами администратора для входа в клиент клиента с проверкой MFA, включая агентов администратора и агентов службы поддержки. В целях тестирования партнеры могут включить [Параметры безопасности Azure AD по умолчанию](https://docs.microsoft.com/azure/active-directory/fundamentals/concept-fundamentals-security-defaults) в клиенте клиента, а затем использовать права делегированного администрирования партнера для доступа к клиенту клиента.
...
- Это очень похоже на ситуацию, когда клиент конечного клиента реализовал MFA для своих администраторов. Например, клиент клиента включил [Параметры безопасности Azure AD](https://docs.microsoft.com/azure/active-directory/fundamentals/concept-fundamentals-security-defaults), что требует, чтобы все учетные записи пользователей с правами администратора вошли в клиент клиента с проверкой MFA, включая агенты администратора и агенты службы поддержки. В целях тестирования партнеры могут включить [Параметры безопасности Azure AD по умолчанию](https://docs.microsoft.com/azure/active-directory/fundamentals/concept-fundamentals-security-defaults) в клиенте клиента, а затем использовать делегированные привилегии администратора для программного доступа к клиенту клиента.
