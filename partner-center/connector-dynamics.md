...
:::image type="content" source="images/cosellconnectors/dynamics-3.png" alt-text="Подключить":::
...
4. Теперь выберите пункт "регистрация в Power Center (Предварительная версия)" и нажмите кнопку **запустить**.
...
- Значение сделки: по умолчанию стоимость сделки из центра партнеров будет синхронизирована с **естиматедвалуе** в CRM и обратно. Если у вас есть другое поле в CRM для синхронизации значения сделки с:
    ...
    а.    Измените имя поля значения сделки в переменной среды Dynamics 365 с помощью имени поля CRM. Обратите внимание, что имя поля следует указывать не на его отображаемом имени.
...
- Код страны для учетной записи клиента. при создании новой ссылки обязательно укажите двухбуквенный код страны (ISO 3166). По умолчанию код страны будет синхронизирован с полем address1_country учетной записи в CRM и обратно. Если в CRM есть другое поле для синхронизации кода страны:
   ...
   а.    Для поля кода страны без подстановки в учетной записи, которая содержит двухбуквенный код:
...
:::image type="content" source="images/cosellconnectors/dynamics-5.png" alt-text="Поле ввода для значений по умолчанию":::
