...
4.  Выберите тип агента и тип администратора. Пользовательский интерфейс Центра партнеров основан на ролях. Поэтому варианты выбора в этом шаге определяют функциональные возможности, которые будут предлагаться пользователю в дальнейшем. Подробнее о возможностях каждой роли см. в разделе [Определение разрешений для пользователей](#setuserpermissions).

5.  Добавьте пользователя. Появится экран подтверждения с временным паролем для нового имени входа в систему. Вы должны скопировать этот пароль и отправить его новому пользователю — он недоступен после закрытия экрана. При первом входе в систему им будет предложено обновить свой пароль.

### <a href="" id="setuserpermissions"></a>Определение разрешений для пользователей

Центр партнеров имеет пользовательский интерфейс, основанный на ролях. Это значит, что вы можете настроить представление пользователя на отображение только функций, которые будут необходимы пользователю для выполнения его должностных обязанностей. Для каждого пользователя необходимо выбрать два параметра:
...
-   Параметр **администратор** определяет, насколько пользователь контролирует среду Центра партнеров и все остальные службы Майкрософт (учетные записи, профили и запросы в службу поддержки). Этот параметр влияет на внешнюю среду Центра партнеров — администратор выставления счетов может получить доступ ко всем счетам за все службы Майкрософт (даже те, которые не связаны с CSP), а глобальный администратор может получить доступ к учетным записям пользователей и клиентов также за пределами CSP.
    ...
    По умолчанию значением параметра всегда должно быть **Не администратор**, если пользователю действительно не требуется дополнительный доступ для выполнения рабочих обязанностей.
...
<li><p>Работоспособность службы и запросы в службу поддержки</p></li>
<li><p>Администрирование от имени</p></li>
</ul></td>
<td><ul>
