...
---
# <a name="partner-security-compliance-status"></a>Соответствие требованиям к безопасности для партнеров

**Относится к**

- Центр партнеров
- Все партнеры по программе поставщиков облачных решений
- Все консультанты
...
>[!NOTE]
>Для партнеров, которые реализовали MFA с помощью базовой политики защиты пользователей Azure AD, важно отметить, что политика защиты пользователей основана на рисках. Пользователям, защищенным этой политикой, будет предлагаться пройти MFA только при попытках входа с риском (например, пользователь выполняет вход из другого расположения). Кроме того, пользователи, защищенные политикой, имеют 14 дней на регистрацию для использования MFA. Пользователи, которые не выполнили регистрацию для использования MFA, не будут получать запросы защиты MFA в течение 14-дневного периода. Поэтому предполагается, что метрики не будут показывать 100 % для партнеров, которые реализовали MFA с помощью базовой политики защиты пользователей Azure AD.
...
### <a name="identity-which-users-have-logged-into-partner-center-without-mfa"></a>Определение пользователей, который входили в Центр партнеров без проверки MFA
